...
public class AccessUtils {
    ...
    private static final String TAG = AccessUtils.class.getSimpleName();
...
//            List<AccessibilityWindowInfo> list2 = mService.getWindows();
//            LogUtils.d(list2.get(0).getRoot().toString());
    ...
    //通过描述查找
    public static AccessibilityNodeInfo.AccessibilityAction findNodeByContent(){
        if (mService.getRootInActiveWindow()!=null){
            List<AccessibilityNodeInfo.AccessibilityAction> list = mService.getRootInActiveWindow().getActionList();
            if (list==null||list.isEmpty()){
                return null;
            }
            for (int i=0;i<list.size();i++){
                LogUtils.d("控件"+i+list.get(i).toString());
            }
            return list.get(0);
        }
        return null;
    ...
    public static void performBack() {
        //默认使用MainService
        if (AccessUtils.mService == null) {
            return;
        }
        AccessUtils.mService.performGlobalAction(AccessibilityService.GLOBAL_ACTION_BACK);
    }
